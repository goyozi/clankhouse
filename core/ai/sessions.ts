...
export class AISessions {
    ...
    async *stream(id: string, options?: { afterMessageId?: string }): AsyncGenerator<AISessionMessage, void, void> {
        ...
        if (options?.afterMessageId !== undefined) {
            ...
            if (!row || row.session_id !== id) throw new Error(`Message not found in session ${id}: ${options.afterMessageId}`);
            lastSeq = row.seq;
        }
        let notified = false;
        let wake = deferred();
    ...
    create(options: { kind: "llm" | "coding-agent", provider: string, model: string }): SessionRecorder {
        ...
        sql.insertSession(db, { id, kind: options.kind, provider: options.provider, model: options.model, started_at: nowIso() });
        active.add(id);
        let seq = 0;
        return {
            id,
            addMessage(role, content) {
                sql.insertSessionMessage(db, { id: newId(), session_id: id, seq: seq++, role, content, created_at: nowIso() });
                notify();
            },
            succeed() {
