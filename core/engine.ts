...
export class Engine {
    ...
    private planFresh(workflowName: string, key: string, input: unknown, latest: RunRow | undefined, running: boolean): Plan {
        ...
        switch (latest.status) {
            case "succeeded": return { type: "noopSucceeded", runRow: latest };
            case "interrupted": return { type: "execute", runRow: latest };
            case "failed": throw new Error(`Run "${key}" has failed; rerun with {from} to start a new attempt`);
        }
    }
    ...
    async executeStep<T extends z.ZodTypeAny>(options: ExecuteStepOptions<T>): Promise<z.infer<T>> {
        ...
        if (existing) {
            ...
            id = newId();
            sql.insertStep(db, { id, run_id: ctx.runId, key, name: options.name, seq: ctx.seq.next++, kind: options.kind, started_at: nowIso() });
        }
