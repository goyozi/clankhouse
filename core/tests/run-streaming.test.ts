...
test("stream on a missing run throws", async () => {
    ...
    await expect(loopy.runs.stream("nope").next()).rejects.toThrow(/not found/)
})
...
test("stream with an unknown or foreign fromStepId throws", async () => {
    ...
    await expect(loopy.runs.stream(runA, { fromStepId: "nope" }).next()).rejects.toThrow(/Step not found/)
    // and when streaming with another run's step id
    // then it also throws a step not found error
    await expect(loopy.runs.stream(runA, { fromStepId: foreignStepId }).next()).rejects.toThrow(/Step not found/)
})
...
test("stream observes a failed step being re-executed on resume", async () => {
    ...
    const done = testRun(second, async () => {
