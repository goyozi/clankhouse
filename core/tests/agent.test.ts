...
test("FakeCodingAgent skips the worktree snapshot when snapshots are disabled", async () => {
    ...
    expect(await runGit(worktree.path, ["for-each-ref", "--format=%(refname)", "refs/clankhouse/agent"])).toBe("")
})
...
test("FakeCodingAgent records a void output as JSON", async () => {
    ...
    const step = run.steps.find((candidate) => candidate.kind === "agent")!
    if (step.kind !== "agent") throw new Error("unreachable")
    const finalItem = (await clankhouse.sessions.get(step.sessionId!)).messages.at(-1)!
...
test("snapshotless agent step replay returns stored output without restoring the worktree", async () => {
    ...
    clankhouse.registerWorkflow("test-workflow", workflowOptions, async () => {
    ...
    expect(invocations).toBe(1)
    expect(fs.existsSync(path.join(worktree.path, "src/hello.ts"))).toBe(false)
    const run = await clankhouse.runs.get(secondId)
    const step = run.steps.find((candidate) => candidate.kind === "agent")!
    expect(step.snapshotRef).toBeUndefined()
})
...
test.each([
    ...
    async ({ recordedSnapshot, replayedSnapshot }) => {
        ...
        publishImpl = () => "published"
        const secondId = clankhouse.rerun(firstId, { from: "publish" })
        await expect(runOutput(clankhouse, secondId)).rejects.toThrow(/recorded with snapshots .* replay requested/)

        // then replay fails with the stable mismatch code before invoking the agent or restoring its change
        expect(invocations).toBe(1)
        expect(fs.existsSync(path.join(worktree.path, "src/hello.ts"))).toBe(false)
        expect(await clankhouse.runs.get(secondId)).toMatchObject({ errorCode: "coding_agent_snapshot_mismatch" })
    }
)
...
test("agent step replay fails loudly when its worktree snapshot is missing", async () => {
    ...
    clankhouse.db.prepare("UPDATE steps SET snapshot_ref = NULL WHERE id = ?").run(agentStep.id)
    ...
    const secondId = clankhouse.rerun(firstId, { from: "publish" })
    await expect(runOutput(clankhouse, secondId)).rejects.toThrow(/no worktree snapshot/)
    expect(await clankhouse.runs.get(secondId)).toMatchObject({ errorCode: "coding_agent_snapshot_missing" })
})
...
test("snapshot refs stay distinct for step keys that sanitize to the same string", async () => {
    ...
    await testRun(clankhouse, async () => {
        ...
        await clankhouse.prefix("a", () => agent.run("b", { prompt: "do it", output: outputSchema, worktree }))
        return null
    })
