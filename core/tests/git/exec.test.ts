...
test("execGitRaw reports a stdin pipe failure instead of crashing the process", async () => {
    ...
    expect(result.stderr.toString("utf8")).toContain(process.platform === "win32" ? "EOF" : "EPIPE")
})
