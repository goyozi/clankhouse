...
export function runHandlers(clankhouse: ClankHouse): RunHandlers {
    return {
        async listRuns(request) {
            ...
            try {
                return { runs: (await clankhouse.runs.list(options)).map(toRunMetadata) }
            } catch (error) {
        ...
        async getRun(request) {
            ...
            try {
                return { run: toWorkflowRun(await clankhouse.runs.get(request.runId)) }
            } catch (error) {
                throw toConnectError(error, {
        ...
        async *watchRun(request, context) {
            required(request.runId, "run_id")
            try {
                for await (const item of clankhouse.runs.stream(request.runId, {
        ...
        resumeRun(request) {
            required(request.runId, "run_id")
            try {
                return { runId: clankhouse.resume(request.runId) }
            } catch (error) {
                throw toConnectError(error, {
                    workflow_run_not_found: () => notFound("Workflow run", request.runId),
        ...
        rerunRun(request) {
            ...
            try {
                return { runId: clankhouse.rerun(request.runId, { from: request.fromStepKey }) }
            } catch (error) {
                throw toConnectError(error, {
                    workflow_run_not_found: () => notFound("Workflow run", request.runId),
                    workflow_step_not_found: () =>
