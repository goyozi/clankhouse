...
test("OpenAIModel returns an earlier tagged string after an untagged assistant message", async () => {
    ...
    expect(
        sessionTextMessages((await clankhouse.sessions.get(step.sessionId!)).messages)
            .slice(-2)
...
test("OpenAIModel preserves an empty final text for a root string output", async () => {
    ...
    expect(sessionTextMessages((await clankhouse.sessions.get(step.sessionId!)).messages).at(-1)!.content).toContain(
        "clankhouse_structured_output"
    )
})
...
test.each(["max_output_tokens", "content_filter"] as const)(
    ...
    async (reason) => {
        ...
        if (step.kind !== "llm") throw new Error("unreachable")
        expect(
            sessionTextMessages((await clankhouse.sessions.get(step.sessionId!)).messages).map(
...
test("OpenAIModel rejects and records an official refusal", async () => {
    ...
    if (step.kind !== "llm") throw new Error("unreachable")
    expect(
        sessionTextMessages((await clankhouse.sessions.get(step.sessionId!)).messages).map((message) => [
            message.role,
            message.content
        ])
    ).toEqual([
