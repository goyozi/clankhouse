...
test("listRuns filters by one or many statuses", () => {
    ...
    expect(sql.listRuns(loopy.db, { statuses: ["failed", "interrupted"] }).map(r => r.id).sort()).toEqual(["b", "c"]);
});
...
test("listRuns combines all filters", () => {
    ...
    const { loopy } = tempLoopy();
    sql.insertRun(loopy.db, runRow({ id: "a", key: "k", workflow_name: "wf1", status: "succeeded", started_at: "2026-01-01T00:00:00.000Z" }));
    sql.insertRun(loopy.db, runRow({ id: "b", key: "k", attempt: 2, workflow_name: "wf1", status: "succeeded", started_at: "2026-01-02T00:00:00.000Z" }));
    sql.insertRun(loopy.db, runRow({ id: "c", key: "k", attempt: 3, workflow_name: "wf1", status: "failed", started_at: "2026-01-03T00:00:00.000Z" }));
    ...
    const rows = sql.listRuns(loopy.db, { key: "k", workflowName: "wf1", statuses: ["succeeded"], lastN: 1 });
...
test("resetStep clears output, error, timestamps, and every linkage column", () => {
    ...
    sql.insertSession(loopy.db, { id: "sess-1", kind: "coding-agent", provider: "fake", model: "fake", started_at: "2026-01-01T00:00:00.000Z" });
    sql.insertArtifact(loopy.db, { id: "art-1", run_id: "r", name: "report", file: "artifacts/r/report", kind: "text", mime_type: null, created_at: "2026-01-01T00:00:00.000Z" });
    sql.insertStep(loopy.db, { id: "s", run_id: "r", key: "k", name: "k", seq: 0, kind: "agent", started_at: "2026-01-01T00:00:00.000Z" });
...
test("findDeliverableEvent matches any of the given keys and picks the oldest", () => {
    ...
    const { loopy } = tempLoopy();
    sql.insertEvent(loopy.db, { id: "e1", key: "other", payload: "1", origin: null, emitted_at: "2026-01-01T00:00:00.000Z" });
    sql.insertEvent(loopy.db, { id: "e2", key: "b", payload: "2", origin: null, emitted_at: "2026-01-03T00:00:00.000Z" });
    sql.insertEvent(loopy.db, { id: "e3", key: "a", payload: "3", origin: null, emitted_at: "2026-01-02T00:00:00.000Z" });
...
test("findDeliverableEvent skips consumed events and breaks ties by id", () => {
    ...
    const { loopy } = tempLoopy();
    sql.insertEvent(loopy.db, { id: "e1", key: "a", payload: "1", origin: null, emitted_at: "2026-01-01T00:00:00.000Z" });
    sql.insertEvent(loopy.db, { id: "e2", key: "a", payload: "2", origin: null, emitted_at: "2026-01-01T00:00:00.000Z" });
...
test("findDeliverableEvent prefers the event already consumed by the same consumer", () => {
    ...
    const { loopy } = tempLoopy();
    sql.insertEvent(loopy.db, { id: "e1", key: "a", payload: "1", origin: null, emitted_at: "2026-01-01T00:00:00.000Z" });
    sql.insertEvent(loopy.db, { id: "e2", key: "a", payload: "2", origin: null, emitted_at: "2026-01-02T00:00:00.000Z" });
