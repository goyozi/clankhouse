...
test("waitFor receives an emitted event and records an event step", async () => {
    ...
    const rows = loopy.db.prepare("SELECT key, payload FROM events").all() as { key: string, payload: string }[];
...
test("a second wait on an already-waited key is rejected", async () => {
    ...
    const second = testRun(loopy, async () =>
        (await loopy.waitFor("go", z.object({ n: z.number() }))).n,
        { key: "key-2" });
...
test("an event emitted while the process is down is delivered on resume", async () => {
    ...
    const result = await testRun(second, async () =>
        (await second.waitFor("go", z.object({ n: z.number() }))).n
    );
...
test("a consumed event is not delivered to later waits", async () => {
    ...
    expect(await testRun(loopy, async () =>
        (await loopy.waitFor("go", z.object({ n: z.number() }))).n,
        { key: "key-1" })).toBe(1);
...
test("an event consumed before the wait step persisted is redelivered on resume", async () => {
    ...
    loopy.db.prepare("UPDATE steps SET status = 'interrupted', output = NULL, ended_at = NULL WHERE key = ?").run("wait:go");
...
test("re-emitting on rerun supersedes the previous unconsumed event", async () => {
    ...
    const received = await testRun(loopy, async () =>
        (await loopy.waitFor("result", z.object({ v: z.number() }))).v,
        { key: "consumer" });
